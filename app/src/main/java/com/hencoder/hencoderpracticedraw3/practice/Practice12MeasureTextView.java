...
public class Practice12MeasureTextView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float t1Length = paint1.measureText(text1, 0, text1.length() );
        float t2Length = paint2.measureText(text2, 0, text2.length() );
        float t3Length = paint1.measureText(text3, 0, text3.length() );


        canvas.drawText(text1, 50, 200, paint1);
        canvas.drawText(text2, 50 + t1Length, 200, paint2);
        canvas.drawText(text3, 50 + t1Length + t2Length, 200, paint1);
    }
}
